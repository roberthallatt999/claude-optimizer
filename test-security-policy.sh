...
assert_eq "ssh allowed (the hook still gates every remote write)" "true" "$(jqs "$p" '.permissions.allow | index("Bash(ssh:*)") != null')"
deploy "$p" >/dev/null
assert_eq "ssh allow rule not duplicated on redeploy" "1" "$(jqs "$p" '[.permissions.allow[] | select(. == "Bash(ssh:*)")] | length')"
