...
#   --no-claudeignore   Skip the generated .claudeignore (protected-path deny rules still apply)
...
NO_CLAUDEIGNORE=false          # .claudeignore is generated by default; --no-claudeignore keeps only the deny rules
...
while [[ $# -gt 0 ]]; do
  ...
  case $1 in
    ...
    -h|--help)
      ...
      echo "  --no-claudeignore       Don't generate .claudeignore (deny rules are applied either way)"
...
# --- Stack-aware protected paths --------------------------------------------
# projects/common/protected-paths.conf plus projects/<stack>/protected-paths.conf, in two tiers:
#   [deny]   secrets, credentials, database dumps → Read() rules in settings.local.json AND
#            .claude/hooks/protected-paths.conf, which safety-guard.sh enforces for file
#            tools and Bash alike. A hard stop.
#   [ignore] dependencies, build output, binary assets → .claudeignore only. Never denied:
#            blocking node_modules/ or dist/ breaks ordinary debugging.
# A stack may promote a baseline [ignore] pattern to [deny] (the CMS stacks do this with
# *.sql, where a loose .sql file is a dump rather than a migration).
#
# .claudeignore itself is advisory: Claude Code does not read it (anthropics/claude-code#29455).
# It documents the project's boundaries and serves gitignore-syntax tools; enforcement is
# entirely the deny rules and the hook.
PROTECTED_DENY=()
PROTECTED_IGNORE=()
PROTECTED_DENY_RULES_JSON="[]"

# read_protected_conf <file> — append its [deny]/[ignore] patterns, skipping duplicates.
read_protected_conf() {
  local file="$1" section="" line existing
  [[ -f "$file" ]] || return 0
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    line="${line%"${line##*[![:space:]]}"}"
    ...
    case "$line" in
      ...
      ''|'#'*)    continue ;;
      '['*)       echo -e "  ${YELLOW}⚠${NC}  Unknown section '$line' in $(basename "$(dirname "$file")")/protected-paths.conf — ignored"; section=""; continue ;;
    esac
    case "$section" in
      deny)
        for existing in ${PROTECTED_DENY[@]+"${PROTECTED_DENY[@]}"}; do
          [[ "$existing" == "$line" ]] && continue 2
        done
        PROTECTED_DENY+=("$line") ;;
      ignore)
        for existing in ${PROTECTED_IGNORE[@]+"${PROTECTED_IGNORE[@]}"}; do
          [[ "$existing" == "$line" ]] && continue 2
        done
        PROTECTED_IGNORE+=("$line") ;;
    esac
  done < "$file"
}

# protected_read_rule <pattern> — the settings.local.json deny rule for one pattern.
#   .git/              → Read(**/.git/**)
#   /storage/backups/  → Read(storage/backups/**)
#   credentials*       → Read(**/credentials*)
#   prisma/*.db        → Read(**/prisma/*.db)
protected_read_rule() {
  local pat="$1"
  case "$pat" in
    */)
...
load_protected_paths() {
  ...
  PROTECTED_IGNORE=()
  PROTECTED_DENY_RULES_JSON="[]"
  read_protected_conf "$SCRIPT_DIR/projects/common/protected-paths.conf"
  read_protected_conf "$STACK_DIR/protected-paths.conf"

  # A pattern promoted to [deny] by the stack must not linger in the advisory tier.
  local pat blocked denied advisory=()
  for pat in ${PROTECTED_IGNORE[@]+"${PROTECTED_IGNORE[@]}"}; do
    denied=false
    for blocked in ${PROTECTED_DENY[@]+"${PROTECTED_DENY[@]}"}; do
      [[ "$blocked" == "$pat" ]] && { denied=true; break; }
    done
    [[ "$denied" == true ]] || advisory+=("$pat")
  done
  PROTECTED_IGNORE=(${advisory[@]+"${advisory[@]}"})

  if [[ ${#PROTECTED_DENY[@]} -gt 0 ]] && command -v jq &>/dev/null; then
    PROTECTED_DENY_RULES_JSON=$(for pat in "${PROTECTED_DENY[@]}"; do protected_read_rule "$pat"; echo; done | jq -R . | jq -s .)
  fi
}

# apply_protected_paths — write .claude/hooks/protected-paths.conf and .claudeignore.
# Both go through install_file, so developer edits are kept and the new version is staged.
apply_protected_paths() {
  load_protected_paths
  if [[ ${#PROTECTED_DENY[@]} -eq 0 && ${#PROTECTED_IGNORE[@]} -eq 0 ]]; then
    echo -e "  ${YELLOW}⚠${NC}  No protected-paths.conf found for stack '$STACK' — protected paths NOT applied"
    return 0
  fi

  local work conf ignore_file
  work=$(mktemp -d) || return 0

  conf="$work/protected-paths.conf"
  {
    echo "# Generated by ai-config for stack: $STACK — do not edit (regenerated on --refresh)."
    echo "# Source: claude-optimizer/projects/{common,$STACK}/protected-paths.conf, [deny] tier."
    echo "#"
    echo "# Read by .claude/hooks/safety-guard.sh. These paths are blocked for Read/Edit/Write/Grep"
    echo "# and for Bash commands that name them — not merely hidden from context."
    echo ""
    printf '%s\n' ${PROTECTED_DENY[@]+"${PROTECTED_DENY[@]}"}
  } > "$conf"
  install_file "$conf" "$PROJECT_DIR/.claude/hooks/protected-paths.conf"

  if [[ "$NO_CLAUDEIGNORE" != true ]]; then
    ignore_file="$work/.claudeignore"
    {
      echo "# .claudeignore — generated by ai-config for stack: $STACK"
      echo "#"
      echo "# ADVISORY ONLY. Claude Code does not read this file today"
      echo "# (anthropics/claude-code#29455 is still open). Enforcement lives in:"
      echo "#"
      echo "#   .claude/settings.local.json    permissions.deny → Read() rules for the blocked tier"
      echo "#   .claude/hooks/safety-guard.sh  blocks those paths for file tools AND Bash"
      echo "#"
      echo "# This file records what the project treats as off-limits, and works with other"
      echo "# tools that read gitignore-syntax exclude files. Edit it freely: ai-config keeps"
      echo "# your changes and stages its own version in .claude/ai-config/pending/.claudeignore."
      echo ""
      echo "# --- Blocked: secrets, credentials, database dumps (enforced) ----------------"
      echo ""
      printf '%s\n' ${PROTECTED_DENY[@]+"${PROTECTED_DENY[@]}"}
      echo ""
      echo "# --- Noise: not secret, just not worth loading by default (advisory) ---------"
      echo ""
      printf '%s\n' ${PROTECTED_IGNORE[@]+"${PROTECTED_IGNORE[@]}"}
    } > "$ignore_file"
    install_file "$ignore_file" "$PROJECT_DIR/.claudeignore"
  fi

  rm -rf "$work"
  if [[ "$DRY_RUN" != true ]]; then
    echo -e "  ${GREEN}✓${NC} Protected paths for $STACK: ${#PROTECTED_DENY[@]} blocked, ${#PROTECTED_IGNORE[@]} advisory"
  fi
}
...
merge_security_policy() {
  ...
  if ! merged=$(printf '%s' "$existing" | jq -s --argjson extra "$PROTECTED_DENY_RULES_JSON" "$JQ_SETTINGS_DEFS"'
    .[0] as $e | .[1] as $p |
...
# Rule names by how they deploy. Core rules come from the stack or the common fallback;
# detection-gated rules need their technology. Before 2026-09 only the core, gated, and
# LEGACY_STACK_RULES names were copied — every other rule a stack shipped was skipped.
CORE_RULES="accessibility.md performance.md memory-management.md token-optimization.md sensitive-files.md deployment-safety.md"
DETECTION_GATED_RULES="tailwind-css.md alpinejs.md bilingual-content.md"
LEGACY_STACK_RULES="expressionengine-templates.md craft-templates.md blade-templates.md nextjs-patterns.md laravel-patterns.md markdown-content.md"

# Every rule this stack deploys besides the core and detection-gated ones: all of the stack's
# rules/ plus the shared TypeScript (when detected) and front-end/API rules for JS-framework stacks.
stack_rule_sources() {
  local f name
  for f in "$STACK_DIR"/rules/*.md; do
    [[ -f "$f" ]] || continue
    name=$(basename "$f")
    case " $CORE_RULES $DETECTION_GATED_RULES " in *" $name "*) continue ;; esac
    echo "$f"
  done
  if [[ "$HAS_TYPESCRIPT" == true && ! -f "$STACK_DIR/rules/typescript-patterns.md" ]]; then
    echo "$SCRIPT_DIR/projects/common/rules/typescript-patterns.md"
  fi
  if js_framework_stack; then
    for name in design-system.md api-design.md; do
      if [[ ! -f "$STACK_DIR/rules/$name" ]]; then
        echo "$SCRIPT_DIR/projects/common/rules/$name"
      fi
    done
  fi
  return 0
...
# --refresh: rules older versions never deployed are added when missing and absent from the
# manifest (a recorded-but-missing rule was deleted on purpose), and updated when present.
refresh_stack_rules() {
  local src name dest
  while IFS= read -r src; do
    [[ -n "$src" ]] || continue
    name=$(basename "$src")
    case " $LEGACY_STACK_RULES " in *" $name "*) continue ;; esac
    dest="$PROJECT_DIR/.claude/rules/$name"
    if [[ -e "$dest" || -z "$(manifest_get "$(rel_path "$dest")")" ]]; then
      do_mkdir "$PROJECT_DIR/.claude/rules"
      do_copy "$src" "$PROJECT_DIR/.claude/rules/"
    fi
  done < <(stack_rule_sources)
}
...
code_index_stack() { js_framework_stack; }
...
verify_deployment() {
  ...
  if [[ "$SHARED_POLICY" == true ]]; then
    ...
    if git -C "$PROJECT_DIR" rev-parse --is-inside-work-tree &>/dev/null; then
      for f in .claude/settings.json .claude/hooks/safety-guard.sh .claude/hooks/protected-paths.conf; do
  ...
  # Stack-aware protected paths
  local pp="$PROJECT_DIR/.claude/hooks/protected-paths.conf" stale=0 pat
  if [[ ! -f "$pp" ]]; then
    hc_warn ".claude/hooks/protected-paths.conf missing — the stack's protected paths aren't enforced; run --refresh"
  else
    load_protected_paths
    for pat in ${PROTECTED_DENY[@]+"${PROTECTED_DENY[@]}"}; do
      grep -qxF "$pat" "$pp" || stale=$((stale + 1))
    done
    if [[ $stale -gt 0 ]]; then
      hc_warn "$stale protected path(s) for '$STACK' missing from .claude/hooks/protected-paths.conf — run --refresh"
    else
      hc_ok "Protected paths current for $STACK (${#PROTECTED_DENY[@]} blocked)"
    fi
    if [[ -x "$guard" ]] && printf '%s' '{"tool_name":"Read","tool_input":{"file_path":"backup/site.sqlite"}}' \
      | (cd "$PROJECT_DIR" && CLAUDE_PROJECT_DIR="$PROJECT_DIR" bash "$guard" 2>/dev/null) | grep -q '"permissionDecision":"deny"'; then
      hc_ok "safety-guard.sh blocks a test database dump"
    else
...
do_uninstall() {
  ...
  # The deny rules to withdraw come from the conf this project was actually deployed with,
  # so the right stack's rules go even when --uninstall runs without --stack. Read before
  # step 1 removes the file.
  local pp="$PROJECT_DIR/.claude/hooks/protected-paths.conf" protected_rules="[]" pat
  if [[ -f "$pp" ]] && command -v jq &>/dev/null; then
    protected_rules=$(while IFS= read -r pat || [[ -n "$pat" ]]; do
      pat="${pat%$'\r'}"
      # No `case` here: bash 3.2 mis-parses a case pattern's `)` inside $( ).
      [[ -z "$pat" || "$pat" == '#'* || "$pat" == '['* ]] && continue
      protected_read_rule "$pat"; echo
    done < "$pp" | jq -R . | jq -s .)
  fi
  ...
  for settings in "$PROJECT_DIR/.claude/settings.local.json" "$PROJECT_DIR/.claude/settings.json"; do
    ...
    updated=$(jq --slurpfile p "$SECURITY_POLICY_FILE" --argjson extra "$protected_rules" "$JQ_SETTINGS_DEFS"'
      (($p[0].permissions.deny // []) + ($p[0].permissions.ask // []) + $extra) as $pol
      | if .permissions then
...
update_gitignore() {
  ...
  if [[ "$SHARED_POLICY" == true ]]; then
    ...
    claude_entries=("CLAUDE.md" "AGENTS.md" "MEMORY.md" "MEMORY-ARCHIVE.md" ".claude/*" "!.claude/settings.json" "!.claude/hooks/" ".claude/hooks/*" "!.claude/hooks/safety-guard.sh" "!.claude/hooks/protected-paths.conf" ".claude/ai-config/")
...
if [[ -d "$STACK_DIR/rules" ]] || [[ -d "$SCRIPT_DIR/projects/common/rules" ]]; then
  ...
  # Every other stack rule, plus shared TypeScript / front-end / API rules where they apply
  while IFS= read -r rule_src; do
    if [[ -n "$rule_src" ]]; then
      do_copy "$rule_src" "$PROJECT_DIR/.claude/rules/"
    fi
  done < <(stack_rule_sources)
